...
def face_detection(image_to_detect):
    ...
    f_detection_classifier = cv2.CascadeClassifier('model/lbpcascade_frontalface.xml')
...
f_classifier = cv2.face.LBPHFaceRecognizer_create()
...
cv2.imshow("LBPH Tahmin "+name, cv2.resize(classify_image, (500,500)))
cv2.waitKey(0)
